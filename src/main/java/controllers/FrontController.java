...
public class FrontController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        //サーブレットコンテキスト、リクエスト、レスポンスをactionのフィールドに設定
     ...
     * (例：パラメータがaction=Employeeの場合、actions.EmployeeAcionオブジェクト)
     * @param request リクエスト
     * @param response レスポンス
     * @return
     */
    @SuppressWarnings({ "rawtypes", "unchecked" }) //コンパイラ警告を抑制
    private ActionBase getAction(HttpServletRequest request, HttpServletResponse reponse) {
        Class type = null;
        ...
        try {
            ...
            //リクエストパラメータに設定されている"action"の値が不正の場合(例:action=xxxxx等、該当するActionクラスがない場合)
